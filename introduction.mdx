...
| **App SDKs** | `@voightxyz/openai` · `@voightxyz/anthropic` | Wrap your provider SDK in production apps — automatic LLM-call capture | [OpenAI](/ai-apps/openai) · [Anthropic](/ai-apps/anthropic) |
...
- **You're shipping an LLM-powered feature to end-users.** Wrap your OpenAI / Anthropic client with the [App SDKs](/ai-apps/openai). Every call gets captured with full token/cost breakdown. Add [`withTrace`](/ai-apps/tracing) at your request boundary to group calls per request and attribute cost per end-user.
