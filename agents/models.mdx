---
title: Models
description: "The LLM behind your agent. GLM 4.6 by default (sponsored by Z.ai), the MiMo family as an option, and bring-your-own-key on the roadmap."
---

The **model** is the brain doing the thinking inside your [framework](/agents/frameworks). You pick one at deploy time and can change it later.

<Note>
Agents need **tool-calling**. A model that can't call tools can't run the agent loop, so the picker only offers models verified to work end to end.
</Note>

## The lineup

Inference is routed through **Voight's own key** — you don't bring one. Two families are available:

<CardGroup cols={2}>
  <Card title="GLM — by Z.ai" icon="brain">
    The GLM lineup (4.5, 4.6, 4.7, 5.x). Strong general reasoning and tool use. Sponsored by Z.ai.
  </Card>
  <Card title="MiMo — by Xiaomi" icon="microchip">
    Fast, inexpensive, tool-capable, with a very large context window.
  </Card>
</CardGroup>

The **default is GLM 4.6**: balanced, proven in production, and strong at tool use. MiMo stays available as the cheapest per-token option.

Every new account starts with a **welcome inference credit**, and usage past it draws from your prepaid credit balance: you always see what a model costs before you pick it.

## On a Nosana GPU: local, private

Agents hosted on a [Nosana GPU](/agents/gpu-hosting) don't use the sponsored lineup. They run **Hermes 3 8B** (NousResearch's Llama 3.1 8B fine-tune, trained for the Hermes runtime's tool-calling format) **locally on the job's own GPU**, with a 64K context window. Inference never leaves the GPU and no provider key exists in the container. It is the private, verifiable option; for heavy reasoning, the GLM lineup on Voight Cloud is stronger.

## Bring your own key — coming soon

The next phase opens the model choice wide open, with cost billed to your own provider account:

- **BYOK · OpenRouter** — paste your OpenRouter key and pick *any* model in the OpenRouter catalog.
- **BYOK · Direct** — connect a provider key directly: **OpenAI, Anthropic, Gemini, Qwen, Z.ai, or Xiaomi**. Calls go straight to the vendor.

Per-model cost controls land alongside token metering in the same phase.

## Choosing

| You want… | Pick |
| --- | --- |
| The proven, balanced default | **GLM 4.6** (default) |
| The cheapest per-token option | **MiMo v2.5** |
| More reasoning depth | **GLM 5.x** |
| Inference that never leaves a GPU you rent | **Nosana GPU** hosting (Hermes 3 8B, local) |
| A specific frontier model | Wait for **BYOK**, or tell us what you need |

## Next

- [Frameworks](/agents/frameworks) — the engine the model runs inside
- [Channels](/agents/channels) — where the agent works
- [Pricing](/agents/pricing) — what a running agent costs
