...
- **Hosting**: **Voight Cloud** (default, live in about a minute) or **Nosana GPU**: a dedicated GPU running a local model, a few minutes to come up, verifiable on-chain. See [GPU hosting](/agents/gpu-hosting).
...
One click. Your agent is provisioned on its own private runtime and goes live in **about 60 seconds** (a few minutes on a Nosana GPU, while the node loads the model). You'll land in its workspace with a live chat, its status, and its channels.
