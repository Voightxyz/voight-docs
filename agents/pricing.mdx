...
- **Pay in crypto** — top up in **USDC, SOL, or NOS** (Nosana's token) from your account. Email sign-ups get an embedded wallet, so there's no card and no external checkout.
- **GPU hosting**: an agent on a [Nosana GPU](/agents/gpu-hosting) costs the same $15 per 30 days. GPU time is currently included; per-hour metering is planned and the rate will be announced before it turns on.
