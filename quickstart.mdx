...
Wrap your provider client once and every model call is captured automatically — prompts, tokens, cache reads, tool calls, latency, errors. Per-user spend attribution with one extra line of code.
...
The `tags` map drives [per-user spend tracking](/concepts/per-user-spend) — the dashboard's **Users sub-tab** populates with per-customer cost as soon as your first request lands. Same pattern with `@voightxyz/anthropic` — see the [OpenAI](/ai-apps/openai) and [Anthropic](/ai-apps/anthropic) SDK pages for the full surface.
