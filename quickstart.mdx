...
A wizard writes the right config into your coding agent. No code changes.

```bash
npx -y @voightxyz/sdk setup
```

Verified working today: **Claude Code**, **Cursor**, and **Codex**. The wizard auto-detects which one you're running (or pass `--target=claude|cursor|codex`). Claude Code and Cursor are wired through hooks; Codex connects through its native OpenTelemetry export, pointed at Voight's OTLP receiver: see the [Codex guide](/sdk/codex) for details.
